'use strict';
var bluetooth=require('nativescript-bluetooth');

var observableArray = require("tns-core-modules/data/observable-array");
var observable = require("tns-core-modules/data/observable");
var frameModule = require("tns-core-modules/ui/frame");
var dialogs = require("tns-core-modules/ui/dialogs");
var encoding=require('text-encoding');
const decoder = new encoding.TextDecoder();
const EventEmitter = require('events');
const crypto = require('browserify-aes');
const debug = require('debug')('MiBand');
const UUID_BASE = (x) => `0000${x}-0000-3512-2118-0009af100700`

const UUID_SERVICE_GENERIC_ACCESS =    '1800'
const UUID_SERVICE_GENERIC_ATTRIBUTE = '1801'
const UUID_SERVICE_DEVICE_INFORMATION = '180a'
const UUID_SERVICE_FIRMWARE =           UUID_BASE('1530')
const UUID_SERVICE_ALERT_NOTIFICATION = '1811'
const UUID_SERVICE_IMMEDIATE_ALERT =    '1802'
const UUID_SERVICE_HEART_RATE =         '180d'
const UUID_SERVICE_MIBAND_1 =           'fee0'
const UUID_SERVICE_MIBAND_2 =           'fee1'
const sleep = (milliseconds) => {
  return new Promise(resolve => setTimeout(resolve, milliseconds))
}
// This is a helper function that constructs an ArrayBuffer based on arguments
const AB = function() {
  let args = [...arguments];

  // Convert all arrays to buffers
  args = args.map(function(i) {
    if (i instanceof Array) {
      return Buffer.from(i);
    }
    return i;
  })
  // Merge into a single buffer
  let buf = Buffer.concat(args);

  // Convert into ArrayBuffer
  let ab = new ArrayBuffer(buf.length);
  let view = new Uint8Array(ab);

  for (let i = 0; i < buf.length; ++i) {
    view[i] = buf[i];
  }
  return ab;
}
async function readValuebt(myuid,serv,puuid)
{
  var resultat;
 return await bluetooth.read({
    peripheralUUID: myuid,
    serviceUUID: serv,
    characteristicUUID: puuid
  }).then(function(result) {
// fi. a heartrate monitor value (Uint8) can be retrieved like this:
return result.value;
}, function (err) {
console.log("read error: " + err);
});
}
async function writeValuebt(myuid,serv,puuid,valsend)
{
    bluetooth.write({
      peripheralUUID: myuid,
      serviceUUID: serv,
      characteristicUUID: puuid,
      value: valsend
    }).then(function (result) {
      console.log("feedback", 'value written');
    },
    function (errorMsg) {
      console.log("feedbackTimestamp", +errorMsg);
    });
  }
    async function write01Valuebt(myuid,serv,puuid,valsend)
{
bluetooth.write({
  peripheralUUID: myuid,
  serviceUUID: serv,
  characteristicUUID: puuid,
  value: valsend
}).then(function (result) {
  console.log("feedback on written");
},
function (errorMsg) {
  console.log("feedback", errorMsg);
});
}
async function writerepValuebt(myuid,serv,puuid,valsend)
{
    bluetooth.writeWithoutResponse({
      peripheralUUID: myuid,
      serviceUUID: serv,
      characteristicUUID: puuid,
      value: valsend
    }).then(function (result) {
      console.log("feedback on written");
    },
    function (errorMsg) {
      console.log("feedback write", errorMsg);
    }
  );
  }
  async function startnotifbt(myuid,serv,puuid)
 {
bluetooth.startNotifying({
  peripheralUUID: myuid,
  serviceUUID: serv,
  characteristicUUID: puuid,
  onNotify: function(result) {
    return result.value;
  }
}).then(function () {
  console.log("feedback", 'subscribed for notifications');
});
}
async function stopnotifbt(myuid,serv,puuid)
{
bluetooth.stopNotifying({
  peripheralUUID: myuid,
  serviceUUID: serv,
  characteristicUUID: puuid
}).then(function (result) {
  console.log("feedback", 'notification stopped');
}, function(error) {
  console.log("feedback", error);
});
}
function parseDate(buff) {
  let year = buff.readUInt16LE(0),
    mon = buff[2]-1,
    day = buff[3],
    hrs = buff[4],
    min = buff[5],
    sec = buff[6],
    msec = buff[8] * 1000 / 256;
  return new Date(year, mon, day, hrs, min, sec)
}

class MiBand extends EventEmitter {

  static get advertisementService() { return 0xFEE0; }

  static get optionalServices() { return [
    UUID_SERVICE_GENERIC_ACCESS,
    UUID_SERVICE_GENERIC_ATTRIBUTE,
    UUID_SERVICE_DEVICE_INFORMATION,
    UUID_SERVICE_FIRMWARE,
    UUID_SERVICE_ALERT_NOTIFICATION,
    UUID_SERVICE_IMMEDIATE_ALERT,
    UUID_SERVICE_HEART_RATE,
    UUID_SERVICE_MIBAND_1,
    UUID_SERVICE_MIBAND_2,
  ] }

  constructor(peripheral) {
    super();

    this.device = peripheral;
    this.char = {}
    this.madata={}
    // TODO: this is constant for now, but should random and managed per-device
    this.key = new Buffer('30313233343536373839404142434445', 'hex');
    //this.textDec = new TextDecoder();
  }


  async init() {
    //let miband2 = await this.device.getPrimaryService(UUID_SERVICE_MIBAND_2)
    this.char.myserv1=UUID_SERVICE_MIBAND_1
    this.char.myserv2=UUID_SERVICE_MIBAND_2
    this.char.auth = UUID_BASE('0009')
  //  let miband1 = await this.device.getPrimaryService(UUID_SERVICE_MIBAND_1)
    this.char.time =   '2a2b'
    this.char.raw_ctrl = UUID_BASE('0001')
    this.char.raw_data =UUID_BASE('0002')
    this.char.config = UUID_BASE('0003')
    this.char.activ = UUID_BASE('0005')
    this.char.batt =  UUID_BASE('0006')
    this.char.steps =  UUID_BASE('0007')
    this.char.user =  UUID_BASE('0008')
    this.char.event =  UUID_BASE('0010')
    this.char.hmr=UUID_SERVICE_HEART_RATE
    //let hrm = await this.device.getPrimaryService(UUID_SERVICE_HEART_RATE)
    this.char.hmr_ctrl = '2a39'
    this.char.hmr_data = '2a37'

    //let imm_alert = await this.device.getPrimaryService(UUID_SERVICE_IMMEDIATE_ALERT)
    this.char.imm_alert=UUID_SERVICE_IMMEDIATE_ALERT
    this.char.alert = '2a06'

  //  let devinfo = await this.device.getPrimaryService(UUID_SERVICE_DEVICE_INFORMATION)
    this.char.devinfo=UUID_SERVICE_DEVICE_INFORMATION
    this.char.info_hwrev ='2a27'
    this.char.info_swrev = '2a28'
    try { // Serial Number is in blocklist of WebBluetooth spec
      this.char.info_serial = '2a25'
    } catch(error) {
       //do nothing
    }

    //let fw = await this.device.getPrimaryService(UUID_SERVICE_FIRMWARE)
    this.char.fw=UUID_SERVICE_FIRMWARE
    this.char.fw_ctrl = UUID_BASE('1531')
    this.char.fw_data = UUID_BASE('1532')

   /*await startnotifbt(this.device,this.char.myserv2,this.char.auth)

    await this.authenticate()

    // Notifications should be enabled after auth
    //for (let char of ['hrm_data', 'event', 'raw_data'])
    {
      await startnotifbt(this.device,this.char.hmr,this.char.hmr_data)
      await startnotifbt(this.device,this.char.myserv1,this.char.batt)
   }*/
  }
/*
  * Authentication
  */

 async authenticate() {
   await this.authReqRandomKey()

   return new Promise((resolve, reject) => {
     setTimeout(() => reject('Timeout'), 10000);
     this.once('authenticated', resolve);
   });
 }

async authSendNewKey(key)
 {
   return await writeValuebt(this.device,this.char.myserv2,this.char.auth,AB([0x01, 0x08], key));
 }
 async authReqRandomKey()
        {
          return await writeValuebt(this.device,this.char.myserv2,this.char.auth,AB([0x02, 0x08]));
        }
 async authSendEncKey(encrypted)
 {
  return await writeValuebt(this.device,this.char.myserv2,this.char.auth,AB([0x03, 0x08], encrypted))
}

 /*
  * Button
  */

 waitButton(timeout = 10000) {
   return new Promise((resolve, reject) => {
     setTimeout(() => reject('Timeout'), timeout);
     this.once('button', resolve);
   });
 }

 /*
  * Notifications
  */
  async moveForward() {
    console.log("writting");
    console.log(this.device+" "+this.char.imm_alert+" "+this.char.alert);
  await bluetooth.writeWithoutResponse({
    serviceUUID: this.char.imm_alert,
    characteristicUUID: this.char.alert,
    peripheralUUID: this.device,
    value: AB([0x01])
  })
}
async testwrite()
{
  console.log("writting ");
await writerepValuebt(this.device,this.char.imm_alert,this.char.alert,AB([0x01, 0x03, 0x19]));
console.log("writting ab vale");
console.log("writeok");
}
 async showNotification(type = 'message') {
   console.log(this.device+" "+this.char.imm_alert+" "+this.char.alert);
   debug('Notification:', type);
   switch(type) {
   case 'message': await writeValuebt(this.device,this.char.imm_alert,this.char.alert,AB([0x01]));   break;
   case 'phone':   await writeValuebt(this.device,this.char.imm_alert,this.char.alert,AB([0x02]));   break;
   case 'vibrate':await writeValuebt(this.device,this.char.imm_alert,this.char.alert,AB([0x03]));   break;
   case 'off':     await writeValuebt(this.device,this.char.imm_alert,this.char.alert,AB([0x00]));   break;
   default:        throw new Error('Unrecognized notification type');
   }
 }

 /*
  * Heart Rate Monitor
  */
  async hrmRead()
  {
     await writeValuebt(this.device,this.char.hmr,this.char.hmr_ctrl,AB([0x15, 0x01, 0x00]))
     await writeValuebt(this.device,this.char.hmr,this.char.hmr_ctrl,AB([0x15, 0x02, 0x00]))
     await writeValuebt(this.device,this.char.hmr,this.char.hmr_ctrl,AB([0x15, 0x02, 0x01]))
     return new Promise((resolve, reject) => {
       setTimeout(() => reject('Timeout'), 15000);
       this.once('heart_rate', resolve);
     });
   }

   async hrmStart()
   {
     await writeValuebt(this.device,this.char.hmr,this.char.hmr_ctrl,AB([0x15, 0x02, 0x00]))
     await writeValuebt(this.device,this.char.hmr,this.char.hmr_ctrl,AB([0x15, 0x01, 0x00]))
     await writeValuebt(this.device,this.char.hmr,this.char.hmr_ctrl,AB([0x15, 0x01, 0x01]))

     // Start pinging HRM
      this.hrmTimer = this.hrmTimer || setInterval(() => {
       debug('Pinging HRM')
        writeValuebt(this.device,this.char.hmr,this.char.hmr_ctrl,AB([0x16]))
     },12000);
   }

   async hrmStop() {
     clearInterval(this.hrmTimer);
     this.hrmTimer = undefined;
     await writeValuebt(this.device,this.char.hmr,this.char.hmr_ctrl,AB([0x15, 0x01, 0x00]))
   }

   /*
    * Pedometer
    */

   async getPedometerStats() {
     var resultat1;
     console.log("thiam "+this.device+" "+this.char.myserv1+" "+this.char.steps);
     await  readValuebt(this.device,this.char.myserv1,this.char.steps).then(function (response)
      {
     let data = Buffer.from(response)
     let result = {}
     //unknown = data.readUInt8(0)
     result.steps = data.readUInt16LE(1)
     //unknown = data.readUInt16LE(3) // 2 more bytes for steps? ;)
     if (data.length >= 8)  result.distance = data.readUInt32LE(5)
     if (data.length >= 12) result.calories = data.readUInt32LE(9)
     resultat1=result
     });
     console.log("thiam "+resultat1);
     return resultat1;
   }

  async getBatteryInfo()
  {
     var resultat1;
     console.log("info ab "+decoder.decode(AB([0x01])));
     await readValuebt(this.device,this.char.myserv1,this.char.batt).then(function (response)
      {
    // On récupère le resultat de la requête dans la varible "response"
    let  data = Buffer.from(response);
    var i;
    var binary='';
    resultat1=data[1];
    if (data.length <= 2) return 'unknown';
    let resultat = {}
    resultat.level = data[1]
    resultat.charging = !!data[2]
    resultat.off_date = parseDate(data.slice(3, 10))
    resultat.charge_date = parseDate(data.slice(11, 18))
    //result.charge_num = data[10]
    resultat.charge_level = data[19]
    resultat1=resultat
   //console.log("mes data " + JSON.stringify(madat));
});
return resultat1;
  }

  async getTime() {
    var resultat1;
    await readValuebt(this.device,this.char.myserv1,this.char.time).then(function (response)
     {
      let data = Buffer.from(response)
      resultat1=data
    });
    return parseDate(resultat1)

  }

    async getSerial() {
      var resultat1;
      await readValuebt(this.device,this.char.devinfo,this.char.info_serial).then(function (response)
       {
               let  data = Buffer.from(response);
               resultat1=data
           });
      return resultat1
    }

    async getHwRevision() {
      var resultat1;
      await readValuebt(this.device,this.char.devinfo,this.char.info_hwrev).then(function (response)
       {
         let  data = Buffer.from(response);
         resultat1=decoder.decode(data)
     });
      if (resultat1.startsWith('V') || resultat1.startsWith('v'))
        resultat1=resultat1.substring(1)
      return resultat1;
    }

    async getSwRevision()
    {
      var resultat1;
      await readValuebt(this.device,this.char.devinfo,this.char.info_swrev).then(function (response)
       {
         let  data = Buffer.from(response);
         resultat1=decoder.decode(data)
     });
      if (resultat1.startsWith('V') || resultat1.startsWith('v'))
        resultat1=resultat1.substring(1)
      return resultat1;
    }

    async setUserInfo(user) {
      let data = new Buffer(16)
      data.writeUInt8   (0x4f, 0) // Set user info command

      data.writeUInt16LE(user.born.getFullYear(), 3)
      data.writeUInt8   (user.born.getMonth()+1, 5)
      data.writeUInt8   (user.born.getDate(), 6)
      switch (user.sex)
      {
      case 'male':   data.writeUInt8   (0, 7); break;
      case 'female': data.writeUInt8   (1, 7); break;
      default:       data.writeUInt8   (2, 7); break;
      }
      data.writeUInt16LE(user.height,  8) // cm
      data.writeUInt16LE(user.weight, 10) // kg
      data.writeUInt32LE(user.id,     12) // id

      await writeValuebt(this.device,this.char.myserv1,this.char.user,AB(data))
    }

    //async reboot() {
    //  await this.char.fw_ctrl.writeValue(AB([0x05]))
    //}

    /*
     * RAW data
     */

    async rawStart() {
      await writeValuebt(this.device,this.char.myserv1,this.char.raw_ctrl,AB([0x01, 0x03, 0x19]))
      await this.hrmStart();
      await writeValuebt(this.device,this.char.myserv1,this.char.raw_ctrl,AB([0x02]))
    }

    async rawStop() {
      await writeValuebt(this.device,this.char.myserv1,this.char.raw_ctrl,AB([0x03]))
      await this.hrmStop();
    }

  /*
   * Internals
   */

     _handleNotify(event) {
       const value = Buffer.from(event.target.value.buffer);

       if (event.target.uuid === this.char.auth.uuid) {
         const cmd = value.slice(0,3).toString('hex');
         if (cmd === '100101') {         // Set New Key OK
           this.authReqRandomKey()
         } else if (cmd === '100201') {  // Req Random Number OK
           let rdn = value.slice(3)
           let cipher = crypto.createCipheriv('aes-128-ecb', this.key, '').setAutoPadding(false)
           let encrypted = Buffer.concat([cipher.update(rdn), cipher.final()])
           this.authSendEncKey(encrypted)
         } else if (cmd === '100301') {
           debug('Authenticated')
           this.emit('authenticated')

         } else if (cmd === '100104') {  // Set New Key FAIL
           this.emit('error', 'Key Sending failed')
         } else if (cmd === '100204') {  // Req Random Number FAIL
           this.emit('error', 'Key Sending failed')
         } else if (cmd === '100304') {
           debug('Encryption Key Auth Fail, sending new key...')
           this.authSendNewKey(this.key)
         } else {
           debug('Unhandled auth rsp:', value);
         }

       } else if (event.target.uuid === this.char.hrm_data.uuid) {
         let rate = value.readUInt16BE(0)
         this.emit('heart_rate', rate)

       } else if (event.target.uuid === this.char.event.uuid) {
         const cmd = value.toString('hex');
         if (cmd === '04') {
           this.emit('button')
         } else {
           debug('Unhandled event:', value);
         }
       } else if (event.target.uuid === this.char.raw_data.uuid) {
         // TODO: parse adxl362 data
         // https://github.com/Freeyourgadget/Gadgetbridge/issues/63#issuecomment-302815121
         debug('RAW data:', value)
       } else {
         debug(event.target.uuid, '=>', value)
       }
     }
}

module.exports = MiBand;
